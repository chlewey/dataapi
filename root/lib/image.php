...
class image extends api {
	function __construct() {
		api::__construct($imagetype,'0.01');
	...
	function fiximage($bd,$fn,$fmt) {
		...
		$ffn = $fbd.$fn;
		if(!file_exists($ffn))
			return $this->status(404,"Image not found.");
		...
		if(!empty($fmt)) {
			...
			if($rfmt != $afmt) {
				...
				if(!file_exists($nfn)) {
					$createimage = "createimagefrom$afmt";
...
class banner extends avatar {
	function __construct($imagetype="avatar") {
		banner::__construct("banner");
	}
}
