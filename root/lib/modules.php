...
function read_module_info($mod,$lines) {
	global $mod_i,$mod_r,$mod_c,$mod_p,$mod_a,$mod_h,$mod_q;
	$mod_i[$mod] = array();
	$mod_c[$mod] = array();
	foreach($lines as $line) {
		...
		case 'hide':
			...
			break;
		case 'dbini':
			if(!isset($mod_q[$mod])) $mod_q[$mod] = array();
			foreach($inst as $sec) {
				$mod_q[$mod][] = $sec;
			}
			break;
...
function commit_module(&$s,$mod) {
	global $mod_x,$mod_i,$mod_c,$mod_r,$mod_q;
	echo "coming to $mod\n";
	print_r($mod_x);
	...
	if(isset($mod_q[$mod]))
		foreach($mod_q[$mod] as $q) {
			if(file_exists($qfn = "mod/$mod/$q.sql")) {
				$qq = file_get_contents($qfn);
				$qq = preg_replace('{--.*}','',$qq);
				$qa = preg_split('{;\r?\n\s*}',$qq);
				foreach($qa as $q) {
					$q = trim($q);
					if(empty($q)) continue;
					if(!il_query($q)) echo il_dberror().chr(10);
				}
			}
			$s.= "# SQL -> $qfn\n";
		}
...
foreach($mod_p as $mod=>$func) {
	$s.= "set_paths('$mod','$func'".(isset($mod_a[$mod])?",'".$mod_a[$mod]."'":'').(in_array($mod,$mod_h)?",'hiden'":"").");\n";
}
$s.= "\nfix_paths();\n?>";
